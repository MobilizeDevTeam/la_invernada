...
class AccountInvoice(models.Model):
    ...
    @api.model
    def _prepare_refund(
        self, invoice, date_invoice=None, date=None, description=None, journal_id=None, tipo_nota=61, mode="0"
    ):
    ...
    @api.multi
    @api.returns("self")
    def refund(self, date_invoice=None, date=None, description=None, journal_id=None, tipo_nota=61, mode="0"):
    ...
    def set_document_class_id(self):
        if self.move_id or self.type in ["in_invoice", "in_refund"]:
            if self.journal_document_class_id.sii_document_class_id.es_factura_compra() or self.nc_compras():
                self._onchange_invoice_line_ids()
            return
...
a VAT."""))
    ...
    def nc_boleta(self):
        ...
        for r in self.referencias:
            return r.sii_referencia_TpoDocRef.es_boleta()
        return False

    def nc_compras(self):
        for r in self.referencias:
            return r.sii_referencia_TpoDocRef.es_factura_compra()

    def _actecos_emisor(self):
        actecos = []
        if self.use_documents and not self.journal_id.journal_activities_ids:
    ...
    def _emisor(self):
        ...
        if not dir_origen.city_id:
            _logger.info('LOG:   company emisora {}'.format(dir_origen))
    ...
    def _receptor(self):
        ...
        if not self.partner_id or Receptor["RUTRecep"] == "66666666-6":
            return Receptor
        if self.use_documents and not self._es_boleta() and not self.nc_boleta():
    ...
    def is_price_included(self):
        ...
        if tax.price_include or (not tax.sii_detailed and (self._es_boleta() or self.nc_boleta())):
            return True
        return False
    ...
    def set_dte_claim(self, claim=False):
        ...
        try:
            if respuesta[key].codResp in [0, 7]:
                self.claim = claim
        except Exception as e:
            msg = "Error al setear Reclamo DTE en odoo : {}".format(str(e))
            _logger.warning(msg)
